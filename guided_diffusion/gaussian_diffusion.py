...
class GaussianDiffusion:
    ...
    # def compare_cond_uncond_diff(self,shape,compare_t,use_classifier=True,**model_kwargs):#主函数 sample
    #     print('Start generating...')
    #     logger_list=[]
    #     if model_kwargs == None:
    #         model_kwargs = {}
    #     x_t = th.randn(shape, device = self.device)
    #     tlist = th.ones([x_t.shape[0]], device = self.device) * self.T
    #     for _ in tqdm(range(self.T),dynamic_ncols=True):
    #         tlist -= 1
    #         if not isinstance(compare_t,list):
    #             compare_t=[compare_t]
    #         if tlist[0] in compare_t:
    #             sum_eps_condition,eps_uncond=self.calc_diff_v2(x_t,tlist,use_classifier)
